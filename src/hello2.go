...
//
//	uint  无    uint       32或64       4或8          与...
//	      无    uint8      8            1
//	      无    uint16     16           2
//	      无    uint32     32           4             相当于32位系统下的uint类型
//	      无    uint64     64           8             相当于64位系统下的uint类型
//	*/
//	//解释 以int8 和uint8举例e,8代表8个bit，能表示的数值个数有2^8=256.
//	//uint8是无符号,能表示的都是正数，0-255,刚好256个数.
//	//int是有符号，既可以正数,也可以负数.-128-127,刚好256个数.
//	//int8 int16 int32 int64 这几个类型的最后都有一个数值,这表明了它们能表示的数值个数是固定的
//	//而int并没有指定它的位数，说明它的大小，是可以变化的
//	//1.当你在32位的系统下，int和uint都占用4个字节，也就是32位.
//	//2.当你在64位的系统下，int和uint都占用8个字节,也就是64位.
//	//出于这个原因，在某些场景下，你应当避免使用int和uint，而使用更加精确的int32和int64，比如在二进制传输，读写文件的结构描述（为了保持文件的
